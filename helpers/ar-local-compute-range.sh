...
until [ "$currentdate" == "$loopenddate" ]; do
  ...
  /usr/libexec/ar-local-compute/lib/mongo-date_delete.py $RUN_DATE $mongoDB_host $mongoDB_port
  ...
  ### run calculator.pig
  pig -x local -useHCatalog -param in_date=$RUN_DATE \
      ...
      -param out_path=/usr/libexec/ar-local-compute/output/ \
