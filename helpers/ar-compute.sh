...
### prepare poems
echo "Preparing poem profiles for $RUN_DATE"
grep ROC_CRITICAL poem_sync_$RUN_DATE_UNDER.out \
     | cut -d $(echo -e '\x01') --output-delimiter=$(echo -e '\x01') -f "3 4 5 6" \
     | grep "ch.cern.sam.ROC_CRITICAL" \
     | sort -u \
     | awk 'BEGIN {ORS="|"; RS="\n"} {print $0}' \
     | gzip -c \
     | base64 \
...
cat sites_$RUN_DATE_UNDER.out.clean \
    | sed 's/\x01/ /g' \
    | grep " SRM " \
    | sed 's/ SRM / SRMv2 /g' \
    | sed 's/ /\x01/g' \
    >> sites_$RUN_DATE_UNDER.out.clean
cat sites_$RUN_DATE_UNDER.out.clean \
    | awk 'BEGIN {ORS="|"; RS="\r\n"} {print $0}' \
    | gzip -c \
    | base64 \
    | awk 'BEGIN {ORS=""} {print $0}' \
    > sites_$RUN_DATE_UNDER.zip
