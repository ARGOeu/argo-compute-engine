...
public class EndpointTimelines extends EvalFunc<Tuple> {
	...
	public EndpointTimelines(String fnOps, String fnDown, String fnAvProfiles, String fnMetricProfiles,
			String targetDate, String fsUsed, String sPeriod, String sInterval) throws IOException {
		...
		// set the Structures
		this.endpointAggr = new DAggregator(this.sPeriod, this.sInterval); // Create
																			// Aggregator
																			// according
																			// to
																			// sampling
	...
	public Tuple exec(Tuple input) {
		...
		this.endpointAggr.aggregate(this.avMgr.getMetricOp(aprofile), this.opsMgr); // should
																					// be
																					// supplied
																					// on
																					// outside
		...
		if (downPeriod != null) {
			...
			try {
				this.endpointAggr.aggregation.fill(this.opsMgr.getDefaultDownInt(), downPeriod.get(0),
						downPeriod.get(1), this.targetDate);
			} catch (ParseException e) {
				LOG.error(e);
			}
		}
